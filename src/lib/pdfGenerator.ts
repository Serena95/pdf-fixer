...
export function generatePDF(data: PdfData) {
  ...
  if (data.obiettiviTable && data.obiettiviTable.length > 0) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8);
    const nota = "Garanzia di risultato minimo: si considera raggiunto al verificarsi di UNA SOLA delle voci della tabella obiettivi (non di tutte e due).";
    const notaLines = doc.splitTextToSize(nota, contentW);
    doc.text(notaLines, marginL, y);
    y += notaLines.length * 4 + 2;
    const nota2 = "L'importo contrattuale è riferibile all'attività riconducibile ad un massimo di 3 bandi l'anno.";
    const nota2Lines = doc.splitTextToSize(nota2, contentW);
    doc.text(nota2Lines, marginL, y);
    y += nota2Lines.length * 4;
